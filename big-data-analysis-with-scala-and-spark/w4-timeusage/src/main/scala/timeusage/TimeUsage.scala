...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val respondentId = StructField(columnNames.head, StringType, nullable = false)
    val otherFields = columnNames.tail.map(f => StructField(f, DoubleType, nullable = false))
    StructType(respondentId :: otherFields)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeedsFilter = (s: String) => List("t01", "t03", "t11", "t1801", "t1803").exists(c => s.startsWith(c))
    val primaryNeeds = columnNames.filter(primaryNeedsFilter)
    val workingActivitiesFilter = (s: String) => List("t05", "t1805").exists(c => s.startsWith(c))
    val workingActivities = columnNames.filter(workingActivitiesFilter)
    val otherActivitiesFilter = (s: String) => List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18").exists(c => s.startsWith(c))
    val otherActivities = columnNames.diff(primaryNeeds).diff(workingActivities).filter(otherActivitiesFilter)
    (primaryNeeds.map(c => col(c)), workingActivities.map(c => col(c)), otherActivities.map(c => col(c)))
  }
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map(r => TimeUsageRow(r.getAs("working"),
                                             r.getAs("sex"),
                                             r.getAs("age"),
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    val avgPrimaryNeeds = round(typed.avg[TimeUsageRow](_.primaryNeeds), 1).as("primaryNeeds").as[Double]
    val avgWork = round(typed.avg[TimeUsageRow](_.work), 1).as("work").as[Double]
    val avgOther = round(typed.avg[TimeUsageRow](_.other), 1).as("other").as[Double]
