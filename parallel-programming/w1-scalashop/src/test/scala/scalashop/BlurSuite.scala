...
class BlurSuite extends FunSuite {
  ...
  test("VerticalBox.parBlur with radius 1 and 4 tasks should correctly blur the entire 3x3 image") {
    val w = 3
    val h = 3
    val src = new Img(w, h)
    val dst = new Img(w, h)
  ...
  test("HorizontalBoxBlur.parBlur with radius 2 and 4 tasks should correctly blur the entire " +
    "4x3 image") {
    val w = 4
    val h = 3
    val src = new Img(w, h)
