...
object LineOfSight {
  ...
  def upsweep(input: Array[Float], from: Int, end: Int, threshold: Int): Tree = {
    if (end - from <= threshold) Leaf(from, end, upsweepSequential(input, from, end))
      ...
      val (t1, t2) = parallel(upsweep(input, from, middle, threshold), upsweep(input, middle, end, threshold))
      Node(t1, t2)
    }
  }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float, tree: Tree): Unit = {
    tree match {
      ...
      case n @ Node(tl, tr) => parallel(
                                 downsweep(input, output, startingAngle, tl),
                                 downsweep(input, output, n.maxPrevious, tr)
                               )
    }
  }
  ...
  def parLineOfSight(input: Array[Float], output: Array[Float], threshold: Int): Unit = {
    val tmp = upsweep(input,0, input.length, threshold)
    downsweep(input, output, 0.0f, tmp)
  }
}
