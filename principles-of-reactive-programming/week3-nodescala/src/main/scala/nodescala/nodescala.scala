...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    if (token.isCancelled || response.isEmpty) exchange.close
    else {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val listenerSubscription = httpListener.start()
    def businessLogic = (cancellationToken: CancellationToken) => async {
        while (cancellationToken.nonCancelled) {
          val (request, exchange) = await {httpListener.nextRequest()}
          respond(exchange, cancellationToken, handler(request))
        }
      }
    val businessLogicSubscription = Future.run()(businessLogic)
    Subscription(listenerSubscription, businessLogicSubscription)
  }
