...
//    // TO IMPLEMENT
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe {
      tryValue => tryValue match {
          case Success(suggestions) => suggestionList.listData = suggestions; println(suggestionList.listData)
          case Failure(exception) => status.text = exception.getMessage
        }
    }

    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks filter (click => !suggestionList.selection.items.isEmpty) flatMap (click => Observable.from(suggestionList.selection.items))
    ...
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      tryValue => tryValue match {
        case Success(pageContent) => editorpane.text = pageContent; println(pageContent)
        case Failure(exception) => status.text = exception.getMessage
      }
    }
